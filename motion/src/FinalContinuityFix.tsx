...
const Mark: React.FC<{kind: Kind; size?: number}> = ({kind, size = 34}) => {
  ...
  return <div style={{width:size,height:size,borderRadius:Math.round(size*.32),display:'grid',placeItems:'center',overflow:'hidden',background:bg,border:kind==='gmail'?'1px solid rgba(20,28,24,.08)':'none',boxShadow:'0 5px 14px rgba(11,19,17,.15)'}}>
...
const notifications: Array<{kind:Kind;x:number;y:number;width:number;start:number;depth:number;seed:number}> = [
...
const CameraWorld: React.FC<{children:React.ReactNode}> = ({children}) => {
  ...
  return <div style={{position:'absolute',left:0,top:0,width:WORLD_WIDTH,height:720,transform:`translate3d(${x}px,${y}px,0) scale(${scale})`,transformOrigin:'640px 360px',pointerEvents:'none',zIndex:116}}>{children}</div>;
};

const TransitionLogos: React.FC = () => {
  const frame = useCurrentFrame();
  if (frame < 108 || frame > 222) return null;
  const fadeIn = interpolate(frame,[108,118],[0,1],clamp);
  ...
  return <CameraWorld>{notifications.map((n,i)=>{
    ...
    return <div key={i} style={{position:'absolute',left:Math.round(n.x+driftX+internalX),top:Math.round(n.y+driftY+interpolate(reveal,[0,1],[16,0])+internalY),width:n.width,minHeight:66,opacity:reveal*fadeIn*sceneFade,transform:`scale(${scale})`,transformOrigin:'left center'}}><div style={{position:'absolute',left:11,top:16}}><Mark kind={n.kind}/></div></div>;
  })}</CameraWorld>;
};

const TransitionPhoneLogos: React.FC = () => {
  const frame = useCurrentFrame();
  if (frame < 108 || frame > 222) return null;
  const fadeIn=interpolate(frame,[108,118],[0,1],clamp);
  const sceneFade=interpolate(frame,[150,222],[1,0],clamp);
  ...
  return <CameraWorld><div style={{position:'absolute',left:150+internalX,top:89+phoneFloat+internalY,width:284,height:548,opacity:phoneReveal*fadeIn*sceneFade,transform:`perspective(1250px) translateX(${interpolate(phoneReveal,[0,1],[-82,0])}px) rotateY(-10deg) rotateX(1.5deg) rotateZ(-3.1deg) scale(${interpolate(phoneReveal,[0,1],[.94,1])})`,transformStyle:'preserve-3d'}}>{rows.map(([kind,top],i)=><div key={i} style={{position:'absolute',left:38,top}}><Mark kind={kind} size={30}/></div>)}</div></CameraWorld>;
};

export const FinalContinuityFix: React.FC = () => {
  const frame=useCurrentFrame();
  const galleryOn=frame>=324&&frame<=520;
  return <AbsoluteFill style={{pointerEvents:'none'}}>
    {frame>=108&&frame<=222 ? <style>{`
      div[style*="z-index: 68"], div[style*="z-index: 71"], div[style*="z-index: 72"], div[style*="z-index: 74"] { opacity: 0 !important; }
    `}</style>:null}
    {galleryOn ? <style>{`
      div[style*="z-index: 8"][style*="filter: blur(3px)"],
      div[style*="z-index: 8"][style*="filter: blur(6px)"] { opacity: 0 !important; }
      div[style*="width: 430px"][style*="height: 630px"][style*="z-index: 24"],
      div[style*="width: 430px"][style*="height: 630px"][style*="z-index: 26"],
      div[style*="width: 430px"][style*="height: 630px"][style*="z-index: 28"] { top: 34px !important; }
    `}</style>:null}
    {galleryOn ? <div style={{position:'absolute',left:0,right:0,top:0,height:82,zIndex:105,background:'linear-gradient(180deg,#D6D8D4 0%,rgba(216,218,214,.98) 58%,rgba(216,218,214,0) 100%)'}}/>:null}
    <TransitionLogos/>
    <TransitionPhoneLogos/>
  </AbsoluteFill>;
};
